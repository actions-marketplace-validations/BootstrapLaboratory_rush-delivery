...
test("canonical OCI metadata validates against root and v0.9.1 schemas", async () => {
  const schemaCases = [
    {
  ...
  for (const { metadataPath, schemaName } of schemaCases) {
    ...
    const schemaUrl = `https://bootstraplaboratory.github.io/rush-delivery/schemas/v0.9.1/${schemaName}`;
    assert.match(
      source,
      new RegExp(
    ...
    for (const schemaDirectory of ["schemas", "schemas/v0.9.1"]) {
