...
| Registry authentication denied                                               | Publication or Cosign registry access | Check token lifetime, username form, repository scope, and clock using the provider control plane.                                                                                                                                                                                  | An authoritative pre-upload `401/403` is normally non-mutating; if the failure happened after an upload began, inspect before retry.                                                    |
...
An authentication denial known to occur before any upload can be retried after
credentials are fixed. When the client only reports generic “registry
publication” failure, or an upload/manifest request may have begun, inspect the
registry first.
